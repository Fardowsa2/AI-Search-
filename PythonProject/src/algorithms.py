# src/algorithms.py - FIXED HEURISTIC USAGE
...
def greedy_best_first(G: nx.Graph, start: str, goal: str, h) -> SearchResult:
    ...
    # Verify the heuristic function works
    try:
        test_h = h(start)
        print(f"Debug: Heuristic test - h({start}) = {test_h}")
...
def astar(G: nx.Graph, start: str, goal: str, h) -> SearchResult:
    ...
    # Initialize: (f_score, g_score, node)
    g_score = {start: 0.0}
    ...
    def _run():
        nonlocal nodes_expanded
        while open_set:
            frontier_sizes.append(len(open_set))
